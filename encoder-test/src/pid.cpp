...
PIDController::PIDController(PIDGains gains_, double integralCap_, double maxJerk_, bool deleteIntegral_, double acceptableError_) {
    gains = gains_;
...
double PIDController::tick(double error) {
    ...
    if (deleteIntegral && error == 0 && derivative == 0) {
        integral = 0;
    }
    ...
    double correctionSignal = nonDerivativeCorrectionSignal + derivativeContribution;
